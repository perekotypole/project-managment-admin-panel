  ...
  try {
    ...
    const list = await Promise.all(
      (await Project.find({})
        ...
        ...el.toObject(),
        errorsCount: await Error.countDocuments({ projectID: el.id }),
      })),
    );
    ...
    for (let i = 0,
      ...
      if (el?.stopped) {
        ...
        if (!el?.status) {
          list.splice(i, 1);
          list.splice(nonWorkingIndex, 0, el);
          nonWorkingIndex += 1;
        }
...
router.post('/switchRunningStatus', dataAccess, async (req, res) => {
  try {
    ...
    if (status) { checkerAddProject(project); } else { checkerRemoveProject(project.token); }
...
router.post('/error/removeAll', dataAccess, async (req, res) => {
  try {
    ...
    if (id) {
      ...
      const projectsIDs = await Project.find({}).distinct('_id');
      await Error.deleteMany({ projectID: { $nin: projectsIDs } });
    } else {
