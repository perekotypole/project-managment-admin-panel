...
const defaultFill = () => {
  const autofill = new Array(numberOfData).fill(null).map(() => ({ usage: 0, time: '' }));
...
const Resources = () => {
  ...
  const getRealtimeData = async () => {
    ...
    setData((prev) => {
      let prevData = prev;
      if (!prevData) prevData = defaultFill();

      return {
        cpu: [...prevData.cpu, cpu].slice(-numberOfData),
        memory: [...prevData.memory, memory].slice(-numberOfData),
        network: [...prevData.network, network].slice(-numberOfData),
      };
    });
  };
  ...
  const getData = async () => {
    ...
    setData({
      cpu: resources?.cpu
        ? [...resources.cpu.history, resources.cpu.current].map((el) => ({
      ...
      memory: resources?.memory
        ? [...resources.memory.history, resources.memory.current].map((el) => ({
      ...
      network: resources?.network
        ? [...resources.network.history, resources.network.current].map((el) => ({
          usage: +el.usage,
          time: formatDate(new Date(el.time)),
        }))
        : [],
